...
object MPsTable : Table("members_of_parliament") {

    val id = varchar("id", 30) // 이름_생년월일

    val extraId = varchar("extra_id", 20).uniqueIndex() // 국회의원 코드 (UNIQUE)

    val name = varchar("name", 50) // 국회의원명
    ...
    val partyName = varchar("party_name", 500) // 정당명

    val electoralDistrictName = varchar("electoral_district_name", 500).nullable() // 선거구명

    val electoralDistrictDivisionName = varchar("electoral_district_type", 500).nullable() // 선거구 구분명

    val committeeName = text("committee_name").nullable() // 소속 상임위원회명

    val reElectionDivisionName = varchar("re_election_status", 50).nullable() // 재선 구분명

    val electionEraco = varchar("election_number", 100).nullable() // 당선 대수

    val gender = enumerationByName("gender", 2, Gender::class) // 성별 (남/여)
